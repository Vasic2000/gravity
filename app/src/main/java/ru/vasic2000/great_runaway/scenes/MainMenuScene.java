...
public class MainMenuScene extends SceneFW {
    ...
    @Override
    public void update() {
        if(pCoreFW.getTouchListenerFW().getTuchUp(50, 300, 250, 50)) {
            pCoreFW.setScene(new GameScene(pCoreFW));
            UtilResourse.sTouch.play(1);
        }
        if(pCoreFW.getTouchListenerFW().getTuchUp(50, 350, 250, 50)) {
            pCoreFW.setScene(new SettingsScene(pCoreFW));
            UtilResourse.sTouch.play(1);
        }
        if(pCoreFW.getTouchListenerFW().getTuchUp(50, 400, 145, 35)) {
            pCoreFW.setScene(new TopDistance(pCoreFW));
            UtilResourse.sTouch.play(1);
        }
        if(pCoreFW.getTouchListenerFW().getTuchUp(50, 450, 120, 35)) {
